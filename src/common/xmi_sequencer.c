...
static void onNoteOn(XMISequencer *seq, uint8_t channel, uint8_t note,
                     uint8_t vel) {
  printf("NoteOn: channel=0X%X note=%i vel=0X%X\n", channel, note, vel);
}

static uint8_t readVLQ(XMISequencer *seq, uint8_t *data, size_t remaining) {
  if ((data[0] & 0x80) == 0) {
    return 0;
  }
  uint32_t vlq = 0;
  ...
  do {
    ...
    vlq <<= 7;
    vlq |= (val & 0x7f);
  } while ((val & 0x80) && (remaining));
  printf("Delay: len=0X%X (%i) ", vlq, pos);
  for (int i = 0; i < pos; i++) {
    printf("0X%X ", histVal[i]);
  }
  printf("\n");
  return pos;
}
    ...
    if (b & 0x80) { // MIDI CMD
      if (b == 0XFF) {
        ...
        case EventType_NOTE_ON:
          onNoteOn(seq, chan, data[0], data[1]);
          data += 2;
          remaining -= 2;
          int r = readVLQ(seq, data, remaining);
          data += r;
          remaining -= r;
          break;
        default:
