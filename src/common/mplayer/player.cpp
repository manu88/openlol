...
static const unsigned oper_num[18] = {0x0,   0x1,   0x2,   0x8,   0x9,   0xA,
                                      0x10,  0x11,  0x12,  0x100, 0x101, 0x102,
                                      0x108, 0x109, 0x10A, 0x110, 0x111, 0x112};

// ----------------------------------------------------------------------------
OPLPlayer::OPLPlayer(int numChips, ChipType type) : ymfm::ymfm_interface() {
  m_chipType = type;
  if (type == ChipOPL3) {
    m_numChips = numChips;
...
// ----------------------------------------------------------------------------
void OPLPlayer::setGain(double gain) { m_sampleGain = gain; }
    ...
    while (m_samplePos >= 1.0 && samp < numSamples * 2) {
      if (m_hpFilterCoef < 1.0) {
        for (int i = 0; i < 2; i++) {
          ...
          m_hpLastOut[i] =
              m_hpFilterCoef * (m_hpLastOut[i] + m_output.data[i] - lastIn);
...
void OPLPlayer::updateMIDI() {
  ...
  // apply gain and use sample rate in/out ratio to scale all accumulated
  // samples
  ...
  for (auto &voice : m_voices) {
    ...
    if (!voice.on && !voice.justChanged) {
      if (voice.channel->num == channel && voice.note == note &&
        ...
        // don't immediately use it, but make it a high priority candidate for
        // later (to help avoid pop/click artifacts when retriggering a recently
        // off note)
        silenceVoice(voice);
...
OPLVoice *OPLPlayer::findVoice(uint8_t channel, uint8_t note,
  ...
  channel &= 15;
  for (auto &voice : m_voices) {
    if (voice.on && voice.justChanged == justChanged &&
        voice.channel == &m_channels[channel] && voice.note == note) {
      return &voice;
    }
  }
...
std::pair<bool, bool> OPLPlayer::activeCarriers(const OPLVoice &voice) const {
  ...
  if (!patchVoice) {
    ...
    scale[0] =
        (voice.patch->voice[0].conn & 1) && (voice.patch->voice[1].conn & 1);
    scale[1] = true;
  }
...
// ----------------------------------------------------------------------------
void OPLPlayer::updateChannelVoices(int8_t channel,
                                    void (OPLPlayer::*func)(OPLVoice &)) {
  for (auto &voice : m_voices) {
...
// ----------------------------------------------------------------------------
void OPLPlayer::updatePatch(OPLVoice &voice, const OPLPatch *newPatch,
                            uint8_t numVoice) {
  // assign the MIDI channel's current patch (or the current drum patch) to this
  // voice
  ...
  if (voice.patchVoice != &patchVoice) {
    ...
    if (useFourOp(newPatch) != oldFourOp) {
      ...
      if (other && other->patch && useFourOp(other->patch) &&
          !useFourOp(newPatch)) {
        silenceVoice(*other);
      }
    ...
    // kill an existing voice, then send the chip far enough forward in time to
    // let the envelope die off (ROTT: fixes nasty reverse cymbal noises in
    // spray.mid
    //        without disrupting note timing too much for the staccato drums in
    //        fanfare2.mid)
    silenceVoice(voice);
    ...
    if (m_chipType == ChipOPL2) {
      ...
      write(voice.chip, REG_OP_WAVEFORM + voice.op + 3,
            patchVoice.op_wave[1] & 3);
  ...
  // update even for the same patch in case silenceVoice was called from
  // somewhere else on this voice
...
void OPLPlayer::updateVolume(OPLVoice &voice) {
  ...
  static const uint8_t opl_volume_map[32] = {
      80, 63, 40, 36, 32, 28, 23, 21, 19, 17, 15, 14, 13, 12, 11, 10,
...
void OPLPlayer::updateFrequency(OPLVoice &voice) {
  ...
  int note =
      (!voice.channel->percussion ? voice.note : voice.patch->fixedNote) +
  ...
  write(voice.chip, REG_VOICE_FREQH + voice.num,
        (voice.freq >> 8) | (voice.on ? (1 << 5) : 0));
}
...
// ----------------------------------------------------------------------------
void OPLPlayer::midiControlChange(uint8_t channel, uint8_t control,
                                  uint8_t value) {
  channel &= 15;
  control &= 0x7f;
  value &= 0x7f;
  ...
  if (data[0] == 0x7e) // universal non-realtime
  {
    ...
    if ((address & 0xfff0ff) == 0x401015) // set drum map
      m_channels[channel].percussion = (data[7] != 0x00);
             ...
             !memcmp(data, "\x43\x10\x4c\x00\x00\x7e\x00\xf7", 8)) // Yamaha
  {
    m_midiType = YamahaXG;
  }
}
