...
// ----------------------------------------------------------------------------
MIDTrack::~MIDTrack() { delete[] m_data; }
...
// ----------------------------------------------------------------------------
SequenceMID::SequenceMID() : Sequence() {
  m_type = 0;
  m_ticksPerBeat = 24;
  m_ticksPerSec = 48;
}
...
bool SequenceMID::isValid(const uint8_t *data, size_t size) {
  ...
  if (!memcmp(data, "MThd", 4)) {
    ...
    return true;
  } else if (!memcmp(data, "RIFF", 4) && !memcmp(data + 8, "RMID", 4)) {
    return true;
  }
...
// ----------------------------------------------------------------------------
void SequenceMID::setDefaults() { setTimePerBeat(500000); }
