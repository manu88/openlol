...
bool OPLPatch::load(OPLPatchSet &patches, const uint8_t *data, size_t size) {
  return loadWOPL(patches, data, size) || loadOP2(patches, data, size) ||
         loadAIL(patches, data, size) || loadTMB(patches, data, size);
}

// ----------------------------------------------------------------------------
bool OPLPatch::loadWOPL(OPLPatchSet &patches, const uint8_t *data,
                        size_t size) {
  if (size < 19)
    return false;
...
bool OPLPatch::loadOP2(OPLPatchSet &patches, const uint8_t *data, size_t size) {
  ...
  for (int i = 0; i < 128 + 47; i++) {
    ...
    // flag bit 0 is "fixed pitch" (for drums), but it's seemingly only used for
    // drum patches anyway, so ignore it?
    patch.dualTwoOp = (bytes[0] & 4);
    // second voice detune
    patch.voice[1].finetune =
        OPLPlayer::midiCalcBend((int8_t)(bytes[2] - 128) / 64.0);
...
bool OPLPatch::loadAIL(OPLPatchSet &patches, const uint8_t *data, size_t size) {
  ...
  while (true) {
    ...
    uint32_t patchPos =
        entry[2] | (entry[3] << 8) | (entry[4] << 16) | (entry[5] << 24);
    if (size < patchPos)
      return false;
