...
struct OPLVoice {
  ...
  bool on = false;
  bool justChanged =
      false; // true after note on/off, false after generating at least 1 sample
  uint8_t note = 0;
  uint8_t velocity = 0;
...
public:
  ...
  enum ChipType { ChipOPL, ChipOPL2, ChipOPL3 };
  ...
  // (note: the output of OPLPlayer::generate is a stereo stream regardless of
  // this setting)
...
private:
  ...
  // if no "off" voices are found, steal one using the same patch or MIDI
  // channel
  ...
  void updatePatch(OPLVoice &voice, const OPLPatch *newPatch,
                   uint8_t numVoice = 0);
  ...
  double m_sampleGain = 1.0;
  double m_sampleStep; // ratio of OPL sample rate to output sample rate
                       // (usually < 1.0)
  double
      m_samplePos; // number of pending output samples (when >= 1.0, output one)
  uint32_t m_samplesLeft;             // remaining samples until next midi event
  ymfm::ymf262::output_data m_output; // output sample data
  // if we need to clock one of the OPLs between register writes, save the
  // resulting sample
